...
Mains_init:
	...
	sw	$t1 12($a0) 		# ar
	sw	$a0 12($s0) 
	lw	$a0 12($s0) 		# load [w], class cool.structure.Attribute
	sw	$a0 0($sp) 		# ar: keep left subexp in the stack
	addiu	$sp $sp -4 		# ar
	la	$a0 int_const2 		# 4
	jal	Object.copy 
	...
	sw	$t1 12($a0) 		# ar
	sw	$a0 16($s0) 
	lw	$a0 16($s0) 		# load [x], class cool.structure.Attribute
	sw	$a0 0($sp) 		# ar: keep left subexp in the stack
	addiu	$sp $sp -4 		# ar
	la	$a0 int_const1 		# 2
	jal	Object.copy 
	...
	sw	$t1 12($a0) 		# ar
	sw	$a0 20($s0) 
	lw	$a0 20($s0) 		# load [y], class cool.structure.Attribute
	sw	$a0 0($sp) 		# ar: keep left subexp in the stack
	addiu	$sp $sp -4 		# ar
	la	$a0 int_const1 		# 2
	jal	Object.copy 
...
Main.main:
	...
	la	$a0 Mains_protObj 	# new: explicit
	jal	Object.copy 
	jal	Mains_init 
	la	$a0 str_const3 
	la	$a0 int_const0 		# 0
